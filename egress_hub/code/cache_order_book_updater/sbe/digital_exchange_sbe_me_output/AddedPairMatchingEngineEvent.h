...
#ifndef _DIGITAL_EXCHANGE_SBE_ME_OUTPUT_ADDEDPAIRMATCHINGENGINEEVENT_CXX_H_
#define _DIGITAL_EXCHANGE_SBE_ME_OUTPUT_ADDEDPAIRMATCHINGENGINEEVENT_CXX_H_
...
class AddedPairMatchingEngineEvent
{
private:
...
public:
    ...
    AddedPairMatchingEngineEvent() = default;

    AddedPairMatchingEngineEvent(
        char *buffer,
    ...
    AddedPairMatchingEngineEvent(char *buffer, const std::uint64_t bufferLength) :
        AddedPairMatchingEngineEvent(buffer, 0, bufferLength, sbeBlockLength(), sbeSchemaVersion())
    {
    }
    ...
    AddedPairMatchingEngineEvent(
        ...
        const std::uint64_t actingBlockLength,
        const std::uint64_t actingVersion) :
        AddedPairMatchingEngineEvent(buffer, 0, bufferLength, actingBlockLength, actingVersion)
    {
    }
    ...
    AddedPairMatchingEngineEvent &wrapForEncode(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
        m_buffer = buffer;
        m_bufferLength = bufferLength;
        m_offset = offset;
    ...
    AddedPairMatchingEngineEvent &wrapAndApplyHeader(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
    ...
    AddedPairMatchingEngineEvent &wrapForDecode(
        char *buffer,
        const std::uint64_t offset,
        const std::uint64_t actingBlockLength,
        const std::uint64_t actingVersion,
    ...
    AddedPairMatchingEngineEvent &sbeRewind()
    {
    ...
    {
        AddedPairMatchingEngineEvent skipper(m_buffer, m_offset, m_bufferLength, m_actingBlockLength, m_actingVersion);
        skipper.skip();
    ...
    AddedPairMatchingEngineEvent &seqNum(const std::int64_t value) SBE_NOEXCEPT
    {
    ...
    AddedPairMatchingEngineEvent &timestamp(const std::int64_t value) SBE_NOEXCEPT
    {
        std::int64_t val = SBE_LITTLE_ENDIAN_ENCODE_64(value);
    ...
    AddedPairMatchingEngineEvent &base(const std::uint64_t index, const char value)
    {
        if (index >= 4)
        {
    ...
    AddedPairMatchingEngineEvent &putBase(std::span<const char, N> src) SBE_NOEXCEPT
    {
    ...
    #ifdef SBE_USE_SPAN
    template <typename T>
    AddedPairMatchingEngineEvent &putBase(T&& src)  SBE_NOEXCEPT requires
        (std::is_pointer_v<std::remove_reference_t<T>> &&
         !std::is_array_v<std::remove_reference_t<T>>)
    #else
    AddedPairMatchingEngineEvent &putBase(const char *const src) SBE_NOEXCEPT
    #endif
    {
    ...
    #ifdef SBE_USE_SPAN
    template <std::size_t N>
    AddedPairMatchingEngineEvent &putBase(const char (&src)[N]) SBE_NOEXCEPT
    {
        return putBase(std::span<const char, N>(src));
    }
    #endif

    AddedPairMatchingEngineEvent &putBase(
        const char value0,
        const char value1,
        const char value2,
    ...
    #ifdef SBE_USE_STRING_VIEW
    AddedPairMatchingEngineEvent &putBase(const std::string_view str)
    {
    ...
    #else
    AddedPairMatchingEngineEvent &putBase(const std::string &str)
    {
        const std::size_t srcLength = str.length();
        if (srcLength > 4)
        {
    ...
    AddedPairMatchingEngineEvent &quote(const std::uint64_t index, const char value)
    {
        if (index >= 4)
        {
    ...
    #ifdef SBE_USE_SPAN
    template <std::size_t N>
    AddedPairMatchingEngineEvent &putQuote(std::span<const char, N> src) SBE_NOEXCEPT
    {
    ...
    #ifdef SBE_USE_SPAN
    template <typename T>
    AddedPairMatchingEngineEvent &putQuote(T&& src)  SBE_NOEXCEPT requires
        (std::is_pointer_v<std::remove_reference_t<T>> &&
         !std::is_array_v<std::remove_reference_t<T>>)
    #else
    AddedPairMatchingEngineEvent &putQuote(const char *const src) SBE_NOEXCEPT
    #endif
    {
    ...
    #ifdef SBE_USE_SPAN
    template <std::size_t N>
    AddedPairMatchingEngineEvent &putQuote(const char (&src)[N]) SBE_NOEXCEPT
    {
        return putQuote(std::span<const char, N>(src));
    }
    #endif

    AddedPairMatchingEngineEvent &putQuote(
        const char value0,
        const char value1,
        const char value2,
        const char value3) SBE_NOEXCEPT
    ...
    #ifdef SBE_USE_STRING_VIEW
    AddedPairMatchingEngineEvent &putQuote(const std::string_view str)
    {
        const std::size_t srcLength = str.length();
        if (srcLength > 4)
        {
    ...
    #else
    AddedPairMatchingEngineEvent &putQuote(const std::string &str)
    {
        const std::size_t srcLength = str.length();
        if (srcLength > 4)
        {
...
friend std::basic_ostream<CharT, Traits> & operator << (
    std::basic_ostream<CharT, Traits> &builder, const AddedPairMatchingEngineEvent &_writer)
{
    AddedPairMatchingEngineEvent writer(
        _writer.m_buffer,
        _writer.m_offset,
    ...
    builder << '{';
    builder << R"("Name": "AddedPairMatchingEngineEvent", )";
