...
#ifndef _DIGITAL_EXCHANGE_SBE_ME_OUTPUT_WITHDRAWEVENT_CXX_H_
#define _DIGITAL_EXCHANGE_SBE_ME_OUTPUT_WITHDRAWEVENT_CXX_H_
...
public:
    ...
    WithdrawEvent(char *buffer, const std::uint64_t bufferLength) :
        WithdrawEvent(buffer, 0, bufferLength, sbeBlockLength(), sbeSchemaVersion())
    {
    }
    ...
    WithdrawEvent(
        ...
        const std::uint64_t actingBlockLength,
        const std::uint64_t actingVersion) :
        WithdrawEvent(buffer, 0, bufferLength, actingBlockLength, actingVersion)
    {
    }
    ...
    WithdrawEvent &wrapForEncode(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
        m_buffer = buffer;
        m_bufferLength = bufferLength;
        m_offset = offset;
    ...
    WithdrawEvent &wrapAndApplyHeader(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
    ...
    {
        WithdrawEvent skipper(m_buffer, m_offset, m_bufferLength, m_actingBlockLength, m_actingVersion);
        skipper.skip();
    ...
    WithdrawEvent &seqNum(const std::int64_t value) SBE_NOEXCEPT
    {
    ...
    WithdrawEvent &timestamp(const std::int64_t value) SBE_NOEXCEPT
    {
        std::int64_t val = SBE_LITTLE_ENDIAN_ENCODE_64(value);
    ...
    WithdrawEvent &accountId(const std::int64_t value) SBE_NOEXCEPT
    {
        std::int64_t val = SBE_LITTLE_ENDIAN_ENCODE_64(value);
    ...
    WithdrawEvent &symbol(const std::uint64_t index, const char value)
    {
        if (index >= 4)
        {
    ...
    WithdrawEvent &putSymbol(std::span<const char, N> src) SBE_NOEXCEPT
    {
        static_assert(N <= 4, "array too large for putSymbol");
    ...
    #ifdef SBE_USE_SPAN
    template <typename T>
    WithdrawEvent &putSymbol(T&& src)  SBE_NOEXCEPT requires
        (std::is_pointer_v<std::remove_reference_t<T>> &&
         !std::is_array_v<std::remove_reference_t<T>>)
    #else
    WithdrawEvent &putSymbol(const char *const src) SBE_NOEXCEPT
    #endif
    {
        std::memcpy(m_buffer + m_offset + 24, src, sizeof(char) * 4);
        return *this;
    }

    #ifdef SBE_USE_SPAN
    template <std::size_t N>
    WithdrawEvent &putSymbol(const char (&src)[N]) SBE_NOEXCEPT
    {
        return putSymbol(std::span<const char, N>(src));
    }
    #endif

    WithdrawEvent &putSymbol(
        const char value0,
        const char value1,
        const char value2,
    ...
    {
        char val0 = (value0);
        std::memcpy(m_buffer + m_offset + 24, &val0, sizeof(char));
        char val1 = (value1);
        std::memcpy(m_buffer + m_offset + 25, &val1, sizeof(char));
        char val2 = (value2);
        std::memcpy(m_buffer + m_offset + 26, &val2, sizeof(char));
        char val3 = (value3);
        std::memcpy(m_buffer + m_offset + 27, &val3, sizeof(char));
    ...
    #ifdef SBE_USE_STRING_VIEW
    WithdrawEvent &putSymbol(const std::string_view str)
    {
    ...
    #else
    WithdrawEvent &putSymbol(const std::string &str)
    {
        const std::size_t srcLength = str.length();
        if (srcLength > 4)
        {
    ...
    WithdrawEvent &qty(const std::int64_t value) SBE_NOEXCEPT
    {
        std::int64_t val = SBE_LITTLE_ENDIAN_ENCODE_64(value);
...
friend std::basic_ostream<CharT, Traits> & operator << (
    std::basic_ostream<CharT, Traits> &builder, const WithdrawEvent &_writer)
{
    WithdrawEvent writer(
        _writer.m_buffer,
        _writer.m_offset,
