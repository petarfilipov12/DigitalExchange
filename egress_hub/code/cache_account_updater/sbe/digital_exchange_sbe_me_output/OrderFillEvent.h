...
#ifndef _DIGITAL_EXCHANGE_SBE_ME_OUTPUT_ORDERFILLEVENT_CXX_H_
#define _DIGITAL_EXCHANGE_SBE_ME_OUTPUT_ORDERFILLEVENT_CXX_H_
...
public:
    static constexpr std::uint16_t SBE_BLOCK_LENGTH = static_cast<std::uint16_t>(152);
    static constexpr std::uint16_t SBE_TEMPLATE_ID = static_cast<std::uint16_t>(6);
    static constexpr std::uint16_t SBE_SCHEMA_ID = static_cast<std::uint16_t>(102);
    static constexpr std::uint16_t SBE_SCHEMA_VERSION = static_cast<std::uint16_t>(1);
    static constexpr const char* SBE_SEMANTIC_VERSION = "1";
    ...
    OrderFillEvent(char *buffer, const std::uint64_t bufferLength) :
        OrderFillEvent(buffer, 0, bufferLength, sbeBlockLength(), sbeSchemaVersion())
    {
    }
    ...
    OrderFillEvent(
        ...
        const std::uint64_t actingBlockLength,
        const std::uint64_t actingVersion) :
        OrderFillEvent(buffer, 0, bufferLength, actingBlockLength, actingVersion)
    {
    }
    ...
    OrderFillEvent &wrapForEncode(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
        m_buffer = buffer;
        m_bufferLength = bufferLength;
        m_offset = offset;
    ...
    OrderFillEvent &wrapAndApplyHeader(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
    ...
    {
        OrderFillEvent skipper(m_buffer, m_offset, m_bufferLength, m_actingBlockLength, m_actingVersion);
        skipper.skip();
    ...
    static SBE_CONSTEXPR std::int64_t seqNumNullValue() SBE_NOEXCEPT
    {
        return SBE_NULLVALUE_INT64;
    }

    static SBE_CONSTEXPR std::int64_t seqNumMinValue() SBE_NOEXCEPT
    {
        return INT64_C(-9223372036854775807);
    }

    static SBE_CONSTEXPR std::int64_t seqNumMaxValue() SBE_NOEXCEPT
    {
        return INT64_C(9223372036854775807);
    }
    ...
    SBE_NODISCARD std::int64_t seqNum() const SBE_NOEXCEPT
    {
        std::int64_t val;
    ...
    OrderFillEvent &seqNum(const std::int64_t value) SBE_NOEXCEPT
    {
    ...
    SBE_NODISCARD std::int64_t timestamp() const SBE_NOEXCEPT
    {
        std::int64_t val;
    ...
    OrderFillEvent &timestamp(const std::int64_t value) SBE_NOEXCEPT
    {
        std::int64_t val = SBE_LITTLE_ENDIAN_ENCODE_64(value);
    ...
    OrderFillEvent &base(const std::uint64_t index, const char value)
    {
        if (index >= 4)
        {
    ...
    OrderFillEvent &putBase(std::span<const char, N> src) SBE_NOEXCEPT
    {
    ...
    #ifdef SBE_USE_SPAN
    template <typename T>
    OrderFillEvent &putBase(T&& src)  SBE_NOEXCEPT requires
        (std::is_pointer_v<std::remove_reference_t<T>> &&
         !std::is_array_v<std::remove_reference_t<T>>)
    #else
    OrderFillEvent &putBase(const char *const src) SBE_NOEXCEPT
    #endif
    {
    ...
    #ifdef SBE_USE_SPAN
    template <std::size_t N>
    OrderFillEvent &putBase(const char (&src)[N]) SBE_NOEXCEPT
    {
        return putBase(std::span<const char, N>(src));
    }
    #endif

    OrderFillEvent &putBase(
        const char value0,
        const char value1,
        const char value2,
    ...
    #ifdef SBE_USE_STRING_VIEW
    OrderFillEvent &putBase(const std::string_view str)
    {
    ...
    #else
    OrderFillEvent &putBase(const std::string &str)
    {
        const std::size_t srcLength = str.length();
        if (srcLength > 4)
        {
    ...
    OrderFillEvent &quote(const std::uint64_t index, const char value)
    {
        if (index >= 4)
        {
    ...
    #ifdef SBE_USE_SPAN
    template <std::size_t N>
    OrderFillEvent &putQuote(std::span<const char, N> src) SBE_NOEXCEPT
    {
    ...
    #ifdef SBE_USE_SPAN
    template <typename T>
    OrderFillEvent &putQuote(T&& src)  SBE_NOEXCEPT requires
        (std::is_pointer_v<std::remove_reference_t<T>> &&
         !std::is_array_v<std::remove_reference_t<T>>)
    #else
    OrderFillEvent &putQuote(const char *const src) SBE_NOEXCEPT
    #endif
    {
    ...
    #ifdef SBE_USE_SPAN
    template <std::size_t N>
    OrderFillEvent &putQuote(const char (&src)[N]) SBE_NOEXCEPT
    {
        return putQuote(std::span<const char, N>(src));
    }
    #endif

    OrderFillEvent &putQuote(
        const char value0,
        const char value1,
        const char value2,
        const char value3) SBE_NOEXCEPT
    ...
    #ifdef SBE_USE_STRING_VIEW
    OrderFillEvent &putQuote(const std::string_view str)
    {
        const std::size_t srcLength = str.length();
        if (srcLength > 4)
        {
    ...
    #else
    OrderFillEvent &putQuote(const std::string &str)
    {
        const std::size_t srcLength = str.length();
        if (srcLength > 4)
        {
    ...
    SBE_NODISCARD static const char *fillPriceMetaAttribute(const MetaAttribute metaAttribute) SBE_NOEXCEPT
    {
        switch (metaAttribute)
        {
            case MetaAttribute::PRESENCE: return "required";
    ...
    static SBE_CONSTEXPR std::uint16_t fillPriceId() SBE_NOEXCEPT
    {
        return 5;
    }

    SBE_NODISCARD static SBE_CONSTEXPR std::uint64_t fillPriceSinceVersion() SBE_NOEXCEPT
    {
        return 0;
    }

    SBE_NODISCARD bool fillPriceInActingVersion() SBE_NOEXCEPT
    {
        return true;
    }

    SBE_NODISCARD static SBE_CONSTEXPR std::size_t fillPriceEncodingOffset() SBE_NOEXCEPT
    {
        return 24;
    }

    static SBE_CONSTEXPR std::int64_t fillPriceNullValue() SBE_NOEXCEPT
    {
        return SBE_NULLVALUE_INT64;
    }

    static SBE_CONSTEXPR std::int64_t fillPriceMinValue() SBE_NOEXCEPT
    {
        return INT64_C(-9223372036854775807);
    }

    static SBE_CONSTEXPR std::int64_t fillPriceMaxValue() SBE_NOEXCEPT
    {
        return INT64_C(9223372036854775807);
    }

    static SBE_CONSTEXPR std::size_t fillPriceEncodingLength() SBE_NOEXCEPT
    {
        return 8;
    }

    SBE_NODISCARD std::int64_t fillPrice() const SBE_NOEXCEPT
    {
        std::int64_t val;
    ...
    OrderFillEvent &fillPrice(const std::int64_t value) SBE_NOEXCEPT
    {
        std::int64_t val = SBE_LITTLE_ENDIAN_ENCODE_64(value);
    ...
    OrderFillEvent &fillQty(const std::int64_t value) SBE_NOEXCEPT
    {
        std::int64_t val = SBE_LITTLE_ENDIAN_ENCODE_64(value);
    ...
    SBE_NODISCARD static const char *takerOrderMetaAttribute(const MetaAttribute metaAttribute) SBE_NOEXCEPT
    {
        switch (metaAttribute)
        {
            case MetaAttribute::PRESENCE: return "required";
    ...
    static SBE_CONSTEXPR std::uint16_t takerOrderId() SBE_NOEXCEPT
    {
        return 7;
    }

    SBE_NODISCARD static SBE_CONSTEXPR std::uint64_t takerOrderSinceVersion() SBE_NOEXCEPT
    {
        return 0;
    }

    SBE_NODISCARD bool takerOrderInActingVersion() SBE_NOEXCEPT
    {
        return true;
    }

    SBE_NODISCARD static SBE_CONSTEXPR std::size_t takerOrderEncodingOffset() SBE_NOEXCEPT
    {
        return 40;
    }
...
public:
    SBE_NODISCARD Order &takerOrder()
    {
        m_takerOrder.wrap(m_buffer, m_offset + 40, m_actingVersion, m_bufferLength);
        return m_takerOrder;
    }

    SBE_NODISCARD static const char *makerOrderMetaAttribute(const MetaAttribute metaAttribute) SBE_NOEXCEPT
    {
        switch (metaAttribute)
        {
            case MetaAttribute::PRESENCE: return "required";
    ...
    static SBE_CONSTEXPR std::uint16_t makerOrderId() SBE_NOEXCEPT
    {
        return 8;
    }

    SBE_NODISCARD static SBE_CONSTEXPR std::uint64_t makerOrderSinceVersion() SBE_NOEXCEPT
    {
        return 0;
    }

    SBE_NODISCARD bool makerOrderInActingVersion() SBE_NOEXCEPT
    {
        return true;
    }

    SBE_NODISCARD static SBE_CONSTEXPR std::size_t makerOrderEncodingOffset() SBE_NOEXCEPT
    {
        return 96;
    }
...
public:
    SBE_NODISCARD Order &makerOrder()
    {
        m_makerOrder.wrap(m_buffer, m_offset + 96, m_actingVersion, m_bufferLength);
        return m_makerOrder;
    }

template<typename CharT, typename Traits>
friend std::basic_ostream<CharT, Traits> & operator << (
    std::basic_ostream<CharT, Traits> &builder, const OrderFillEvent &_writer)
{
    OrderFillEvent writer(
        _writer.m_buffer,
        _writer.m_offset,
    ...
    builder << ", ";
    builder << R"("takerOrder": )";
    builder << writer.takerOrder();

    builder << ", ";
    builder << R"("makerOrder": )";
    builder << writer.makerOrder();
