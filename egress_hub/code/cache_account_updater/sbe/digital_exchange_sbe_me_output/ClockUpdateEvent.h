...
#ifndef _DIGITAL_EXCHANGE_SBE_ME_OUTPUT_CLOCKUPDATEEVENT_CXX_H_
#define _DIGITAL_EXCHANGE_SBE_ME_OUTPUT_CLOCKUPDATEEVENT_CXX_H_
...
public:
    ...
    ClockUpdateEvent(char *buffer, const std::uint64_t bufferLength) :
        ClockUpdateEvent(buffer, 0, bufferLength, sbeBlockLength(), sbeSchemaVersion())
    {
    }
    ...
    ClockUpdateEvent(
        ...
        const std::uint64_t actingBlockLength,
        const std::uint64_t actingVersion) :
        ClockUpdateEvent(buffer, 0, bufferLength, actingBlockLength, actingVersion)
    {
    }
    ...
    ClockUpdateEvent &wrapForEncode(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
        m_buffer = buffer;
        m_bufferLength = bufferLength;
        m_offset = offset;
    ...
    ClockUpdateEvent &wrapAndApplyHeader(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
    ...
    ClockUpdateEvent &wrapForDecode(
        char *buffer,
        const std::uint64_t offset,
        const std::uint64_t actingBlockLength,
        const std::uint64_t actingVersion,
    ...
    {
        ClockUpdateEvent skipper(m_buffer, m_offset, m_bufferLength, m_actingBlockLength, m_actingVersion);
        skipper.skip();
    ...
    ClockUpdateEvent &seqNum(const std::int64_t value) SBE_NOEXCEPT
    {
    ...
    ClockUpdateEvent &timestamp(const std::int64_t value) SBE_NOEXCEPT
    {
        std::int64_t val = SBE_LITTLE_ENDIAN_ENCODE_64(value);
...
friend std::basic_ostream<CharT, Traits> & operator << (
    std::basic_ostream<CharT, Traits> &builder, const ClockUpdateEvent &_writer)
{
    ClockUpdateEvent writer(
        _writer.m_buffer,
        _writer.m_offset,
    ...
    builder << '{';
    builder << R"("Name": "ClockUpdateEvent", )";
