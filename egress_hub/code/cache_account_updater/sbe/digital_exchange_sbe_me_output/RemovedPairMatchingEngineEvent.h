...
#ifndef _DIGITAL_EXCHANGE_SBE_ME_OUTPUT_REMOVEDPAIRMATCHINGENGINEEVENT_CXX_H_
#define _DIGITAL_EXCHANGE_SBE_ME_OUTPUT_REMOVEDPAIRMATCHINGENGINEEVENT_CXX_H_
...
class RemovedPairMatchingEngineEvent
{
private:
...
public:
    ...
    RemovedPairMatchingEngineEvent() = default;

    RemovedPairMatchingEngineEvent(
        char *buffer,
    ...
    RemovedPairMatchingEngineEvent(char *buffer, const std::uint64_t bufferLength) :
        RemovedPairMatchingEngineEvent(buffer, 0, bufferLength, sbeBlockLength(), sbeSchemaVersion())
    {
    }
    ...
    RemovedPairMatchingEngineEvent(
        ...
        const std::uint64_t actingBlockLength,
        const std::uint64_t actingVersion) :
        RemovedPairMatchingEngineEvent(buffer, 0, bufferLength, actingBlockLength, actingVersion)
    {
    }
    ...
    RemovedPairMatchingEngineEvent &wrapForEncode(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
        m_buffer = buffer;
        m_bufferLength = bufferLength;
        m_offset = offset;
    ...
    RemovedPairMatchingEngineEvent &wrapAndApplyHeader(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
    ...
    RemovedPairMatchingEngineEvent &wrapForDecode(
        char *buffer,
        const std::uint64_t offset,
        const std::uint64_t actingBlockLength,
        const std::uint64_t actingVersion,
    ...
    RemovedPairMatchingEngineEvent &sbeRewind()
    {
    ...
    {
        RemovedPairMatchingEngineEvent skipper(m_buffer, m_offset, m_bufferLength, m_actingBlockLength, m_actingVersion);
        skipper.skip();
    ...
    RemovedPairMatchingEngineEvent &seqNum(const std::int64_t value) SBE_NOEXCEPT
    {
    ...
    RemovedPairMatchingEngineEvent &timestamp(const std::int64_t value) SBE_NOEXCEPT
    {
        std::int64_t val = SBE_LITTLE_ENDIAN_ENCODE_64(value);
    ...
    RemovedPairMatchingEngineEvent &base(const std::uint64_t index, const char value)
    {
        if (index >= 4)
        {
    ...
    RemovedPairMatchingEngineEvent &putBase(std::span<const char, N> src) SBE_NOEXCEPT
    {
    ...
    #ifdef SBE_USE_SPAN
    template <typename T>
    RemovedPairMatchingEngineEvent &putBase(T&& src)  SBE_NOEXCEPT requires
        (std::is_pointer_v<std::remove_reference_t<T>> &&
         !std::is_array_v<std::remove_reference_t<T>>)
    #else
    RemovedPairMatchingEngineEvent &putBase(const char *const src) SBE_NOEXCEPT
    #endif
    {
    ...
    #ifdef SBE_USE_SPAN
    template <std::size_t N>
    RemovedPairMatchingEngineEvent &putBase(const char (&src)[N]) SBE_NOEXCEPT
    {
        return putBase(std::span<const char, N>(src));
    }
    #endif

    RemovedPairMatchingEngineEvent &putBase(
        const char value0,
        const char value1,
        const char value2,
    ...
    #ifdef SBE_USE_STRING_VIEW
    RemovedPairMatchingEngineEvent &putBase(const std::string_view str)
    {
    ...
    #else
    RemovedPairMatchingEngineEvent &putBase(const std::string &str)
    {
        const std::size_t srcLength = str.length();
        if (srcLength > 4)
        {
    ...
    RemovedPairMatchingEngineEvent &quote(const std::uint64_t index, const char value)
    {
        if (index >= 4)
        {
    ...
    #ifdef SBE_USE_SPAN
    template <std::size_t N>
    RemovedPairMatchingEngineEvent &putQuote(std::span<const char, N> src) SBE_NOEXCEPT
    {
    ...
    #ifdef SBE_USE_SPAN
    template <typename T>
    RemovedPairMatchingEngineEvent &putQuote(T&& src)  SBE_NOEXCEPT requires
        (std::is_pointer_v<std::remove_reference_t<T>> &&
         !std::is_array_v<std::remove_reference_t<T>>)
    #else
    RemovedPairMatchingEngineEvent &putQuote(const char *const src) SBE_NOEXCEPT
    #endif
    {
    ...
    #ifdef SBE_USE_SPAN
    template <std::size_t N>
    RemovedPairMatchingEngineEvent &putQuote(const char (&src)[N]) SBE_NOEXCEPT
    {
        return putQuote(std::span<const char, N>(src));
    }
    #endif

    RemovedPairMatchingEngineEvent &putQuote(
        const char value0,
        const char value1,
        const char value2,
        const char value3) SBE_NOEXCEPT
    ...
    #ifdef SBE_USE_STRING_VIEW
    RemovedPairMatchingEngineEvent &putQuote(const std::string_view str)
    {
        const std::size_t srcLength = str.length();
        if (srcLength > 4)
        {
    ...
    #else
    RemovedPairMatchingEngineEvent &putQuote(const std::string &str)
    {
        const std::size_t srcLength = str.length();
        if (srcLength > 4)
        {
...
friend std::basic_ostream<CharT, Traits> & operator << (
    std::basic_ostream<CharT, Traits> &builder, const RemovedPairMatchingEngineEvent &_writer)
{
    RemovedPairMatchingEngineEvent writer(
        _writer.m_buffer,
        _writer.m_offset,
    ...
    builder << '{';
    builder << R"("Name": "RemovedPairMatchingEngineEvent", )";
