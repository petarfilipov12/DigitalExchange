...
#ifndef _DIGITAL_EXCHANGE_SBE_ME_OUTPUT_DEPOSITEVENT_CXX_H_
#define _DIGITAL_EXCHANGE_SBE_ME_OUTPUT_DEPOSITEVENT_CXX_H_
...
public:
    ...
    DepositEvent(char *buffer, const std::uint64_t bufferLength) :
        DepositEvent(buffer, 0, bufferLength, sbeBlockLength(), sbeSchemaVersion())
    {
    }
    ...
    DepositEvent(
        ...
        const std::uint64_t actingBlockLength,
        const std::uint64_t actingVersion) :
        DepositEvent(buffer, 0, bufferLength, actingBlockLength, actingVersion)
    {
    }
    ...
    DepositEvent &wrapForEncode(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
        m_buffer = buffer;
        m_bufferLength = bufferLength;
        m_offset = offset;
    ...
    DepositEvent &wrapAndApplyHeader(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
    ...
    {
        DepositEvent skipper(m_buffer, m_offset, m_bufferLength, m_actingBlockLength, m_actingVersion);
        skipper.skip();
    ...
    DepositEvent &seqNum(const std::int64_t value) SBE_NOEXCEPT
    {
    ...
    DepositEvent &timestamp(const std::int64_t value) SBE_NOEXCEPT
    {
        std::int64_t val = SBE_LITTLE_ENDIAN_ENCODE_64(value);
    ...
    DepositEvent &accountId(const std::int64_t value) SBE_NOEXCEPT
    {
        std::int64_t val = SBE_LITTLE_ENDIAN_ENCODE_64(value);
    ...
    DepositEvent &symbol(const std::uint64_t index, const char value)
    {
        if (index >= 4)
        {
    ...
    DepositEvent &putSymbol(std::span<const char, N> src) SBE_NOEXCEPT
    {
    ...
    #ifdef SBE_USE_SPAN
    template <typename T>
    DepositEvent &putSymbol(T&& src)  SBE_NOEXCEPT requires
        (std::is_pointer_v<std::remove_reference_t<T>> &&
         !std::is_array_v<std::remove_reference_t<T>>)
    #else
    DepositEvent &putSymbol(const char *const src) SBE_NOEXCEPT
    #endif
    {
    ...
    #ifdef SBE_USE_SPAN
    template <std::size_t N>
    DepositEvent &putSymbol(const char (&src)[N]) SBE_NOEXCEPT
    {
        return putSymbol(std::span<const char, N>(src));
    }
    #endif

    DepositEvent &putSymbol(
        const char value0,
        const char value1,
        const char value2,
    ...
    #ifdef SBE_USE_STRING_VIEW
    DepositEvent &putSymbol(const std::string_view str)
    {
    ...
    #else
    DepositEvent &putSymbol(const std::string &str)
    {
        const std::size_t srcLength = str.length();
        if (srcLength > 4)
        {
    ...
    DepositEvent &qty(const std::int64_t value) SBE_NOEXCEPT
    {
        std::int64_t val = SBE_LITTLE_ENDIAN_ENCODE_64(value);
...
friend std::basic_ostream<CharT, Traits> & operator << (
    std::basic_ostream<CharT, Traits> &builder, const DepositEvent &_writer)
{
    DepositEvent writer(
        _writer.m_buffer,
        _writer.m_offset,
