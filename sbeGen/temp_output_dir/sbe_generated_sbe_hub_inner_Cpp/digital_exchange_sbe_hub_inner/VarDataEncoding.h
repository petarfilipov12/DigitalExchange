...
#ifndef _DIGITAL_EXCHANGE_SBE_HUB_INNER_VARDATAENCODING_CXX_H_
#define _DIGITAL_EXCHANGE_SBE_HUB_INNER_VARDATAENCODING_CXX_H_
...
public:
    ...
    VarDataEncoding(
        ...
        const std::uint64_t bufferLength,
        const std::uint64_t actingVersion) :
        VarDataEncoding(buffer, 0, bufferLength, actingVersion)
    {
    }
    ...
    VarDataEncoding(
        char *buffer,
        const std::uint64_t bufferLength) :
        VarDataEncoding(buffer, 0, bufferLength, sbeSchemaVersion())
    {
    }

    VarDataEncoding &wrap(
        char *buffer,
        const std::uint64_t offset,
        const std::uint64_t actingVersion,
        const std::uint64_t bufferLength)
    ...
    VarDataEncoding &length(const std::uint32_t value) SBE_NOEXCEPT
    {
...
friend std::basic_ostream<CharT, Traits> & operator << (
    std::basic_ostream<CharT, Traits> &builder, VarDataEncoding &writer)
{
    builder << '{';
    builder << R"("length": )";
